...
uint16_t scan_keypad_rising_edge()
{
	...
	uint16_t new_pressed_keys = pressed_keys_tempo  & ~pressed_keys;	// Check new pressed keys
...
int main(void)
{
	...
	while(1)
	{
		while ((input = scan_keypad_rising_edge()) == 0);	// Scan keypad for 1st number
		new_key_1 = keypad_to_ascii(input);		// Turn input to ASCII
		lcd_data(new_key_1);
		
		while ((input = scan_keypad_rising_edge()) == 0);	// Scan keypad for 2nd number
		new_key_2 = keypad_to_ascii(input);			// Turn input to ASCII
		lcd_data(new_key_2);
		
		// Check if input is number 30
		if ((new_key_1 == '3') && (new_key_2 == '0')) light_leds_3s();	// if YES light leds for 3s
		else blink_leds_6s();		// if NO blink leds for 6s
